...
function fallbackTelemetry(): AssetTelemetry[] {
  return SAMPLE_ASSETS.map((asset) => ({
    ...
    oracleConfidence: 0,
    signals: {
      energySource: asset.energySource ?? "UNKNOWN",
      computeArchitecture: asset.computeArchitecture ?? "UNKNOWN",
      networkTopology: asset.topology ?? "UNKNOWN",
      region: asset.region ?? "UNKNOWN",
      complianceStatus: asset.complianceStatus ?? "PENDING_REVIEW",
      maintenanceCostRate: asset.maintenanceCostRate,
      ...
      legalRiskScore: asset.legalRiskScore ?? asset.riskScore,
      aggregateRiskScore: asset.riskScore,
    },
    agency: {
      stage: asset.agencyStage,
      stageIndex: agencyStageIndex(asset.agencyStage),
      description: describeAgencyStage(asset.agencyStage),
