...
export function agencyStageIndex(stage) {
    return stageOrder.indexOf(stage);
}
export function nextAgencyStage(stage) {
    const idx = agencyStageIndex(stage);
    return stageOrder[Math.min(idx + 1, stageOrder.length - 1)];
}
export function describeAgencyStage(stage) {
    switch (stage) {
        case "HUMAN_OWNED":
...
export function estimateAutonomyRisk(asset) {
