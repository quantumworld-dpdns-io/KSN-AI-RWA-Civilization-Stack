...
export function validatePolicy(policy) {
    const total = policy.maintenanceReservePct +
        policy.insurancePct +
...
export function simulateGrossRevenue(asset, epochHours = 24) {
    const computeSold = asset.hashrate * asset.utilization * epochHours;
    return computeSold * asset.revenuePerComputeUnit;
}
export function simulateYieldDistribution(asset, policy = DEFAULT_POLICY, epochHours = 24) {
    ...
    const humanInvestorYield = Math.max(0, grossRevenue - maintenanceReserve - insurancePool - aiTreasury - planetaryDividend - retainedForExpansion);
    return {
        grossRevenue,
        maintenanceReserve,
        insurancePool,
